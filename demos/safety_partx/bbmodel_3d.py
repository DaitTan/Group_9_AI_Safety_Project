...
def run_episode(init_state, init_env_vars, env, model, max_steps_per_episode = 400, render = False):    
    ...
    if render:
        imageio.mimsave("3d_ex1.gif", [np.array(img) for i, img in enumerate(images) if i%2 == 0], fps=29)
...
class CartpoleModel(Model[CartPoleDataT, None]):
    ...
    def simulate(self, inputs: ModelInputs, intrvl: Interval) -> CartPoleResultT:
        ...
        # print(init_state)
        times, traj = run_episode([1e-3, 1e-3, 1e-3, 1e-3], init_env_vars, self.envs, self.model, render=True)
        # print(traj.shape)
...
# with open("Cartpole_PartxRes_3d_best_f_points.pkl", "rb") as f:
#     data = pickle.load(f)
...
# sample1 =  [0.15, 0.6, 2]
# cartpole_blackbox = CartpoleModel(100000)
